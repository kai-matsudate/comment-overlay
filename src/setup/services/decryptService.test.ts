...
import { decryptEnvFile, DecryptError } from './decryptService.js';

// テスト用の暗号化データを生成するヘルパー
// NOTE: テスト専用ユーティリティ - 入力は全てテストコード内でハードコードされた値
function createEncryptedData(content: string, password: string): Buffer {
  // OpenSSL互換の暗号化をシェルで実行
  // シェルエスケープ問題を避けるため、inputオプションでstdinにデータを渡す
  const result = execSync(
    `openssl enc -aes-256-cbc -pbkdf2 -pass pass:${password}`,
    {
      encoding: 'buffer',
