...
- **Slackトークンの管理**: `.env` ファイルに保存されるトークンは機密情報です。Gitリポジトリにコミットしないでください（`.gitignore` で除外済み）
- **ローカル使用前提**: サーバーは `localhost` でのみリッスンします。外部公開は想定していません
- **信頼できる環境での使用**: 同一ホスト上の他のアプリケーションからWebSocket接続が可能です
